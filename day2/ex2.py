...
class CalculatePosition:
    ...
    def new_rule(self):
        ...
        for position in list_of_positions:
            if "forward" in position:
                forward = int(re.findall(r"\d+", position).pop())
                depth = depth + (forward * aim)
            elif "down" in position:
                down = int(re.findall(r"\d+", position).pop())
                aim = aim + down
            elif "up" in position:
                up = int(re.findall(r"\d+", position).pop())
                aim = aim - up
        return depth * self.split_positions()[2]


position = CalculatePosition("sample.txt").new_rule()
print(position)
